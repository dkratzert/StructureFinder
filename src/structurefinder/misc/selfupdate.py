...
#  Dr. Daniel Kratzert
#  ----------------------------------------------------------------------------
"""Self update of a StructureFinder installation on Windows.

StructureFinder downloads the installer of the new version itself and starts it afterward.  An
installation made with ``StructureFinder-setup-x64-vNNN.exe /CURRENTUSER`` lives in
``%LocalAppData%\\Programs\\StructureFinder`` and is replaced by an ordinary process, a machine wide
installation in ``C:\\Program Files`` needs an elevated installer, which is requested with
``ShellExecuteW('runas', ...)``.

StructureFinder cannot end itself before the installer starts, but every loaded ``.exe``/``.pyd``/
``.dll`` keeps a lock in the installation directory.  Therefore the installer is not started
directly but by a small ``.cmd`` launcher that ends all StructureFinder processes with ``taskkill``
...
running QApplication.  The GUI part lives in :mod:`structurefinder.misc.dialogs`.
"""
...
SETUP_URL = 'https://dkratzert.de/files/structurefinder/StructureFinder-setup-x64-v{version}.exe'
CHECKSUM_URL = 'https://dkratzert.de/files/structurefinder/StructureFinder-setup-x64-v{version}-sha512.sha'
# Inno Setup checks this mutex (AppMutex in strf-install_win64.iss) and refuses to
# overwrite the files of a still running StructureFinder:
RUNNING_MUTEX_NAME = 'StructureFinderSetupMutex'
DOWNLOAD_TIMEOUT = 30
BLOCK_SIZE = 65536
# A virus scanner can hold the freshly downloaded installer for a moment:
LOCKED_FILE_ATTEMPTS = 5
LOCKED_FILE_DELAY = 1.0
# Downloads of this age belong to a previous update and not to a second running StructureFinder:
...
PROCESS_NAME = 'structurefinder.exe'
LAUNCHER_SCRIPT_NAME = 'update-structurefinder.cmd'
...
def sha512_checksum_of_file(filename: str, block_size: int = BLOCK_SIZE) -> str:
    """Calculate a SHA512 checksum from a file."""
...
def installation_directory() -> Path:
    """Directory that contains structurefinder.exe and the structurefinder package."""
...
def is_installed() -> bool:
    """False when StructureFinder runs from a source checkout instead of from an installation.

    An installation ships its own python.exe next to the structurefinder package, a checkout runs
...
def is_user_installation() -> bool:
    ...
    elevated StructureFinder may write into ``C:\\Program Files`` as well, while its installer
...
def user_installation_directory() -> Path:
    ...
    return base / 'Programs' / 'StructureFinder'


def is_elevated() -> bool:
    """True if StructureFinder itself runs with administrator rights."""
    if not is_windows():
        return False
...
def _headers() -> dict[str, str]:
    return {'User-Agent': f'StructureFinder v{VERSION} ({sys.platform})'}
...
def download_installer(version: str,
    ...
    target_dir = Path(tempfile.mkdtemp(prefix='structurefinder-update-'))
...
def _remove_leftover_downloads() -> None:
    ...
    Only old directories are touched; a young one may belong to a second StructureFinder that is
    downloading right now or has just started its installer.
    """
    too_old = time.time() - LEFTOVER_DOWNLOAD_AGE
    for directory in Path(tempfile.gettempdir()).glob('structurefinder-update-*'):
...
def start_installer(setup_file: Path) -> None:
    ...
    The launcher ends every running StructureFinder before the installer replaces the files, so
    this process may (and should) quit immediately afterwards (see `structurefinder.misc.dialogs`).
    A machine wide installation is only replaceable by an elevated installer, so the user is
...
def _start_update(setup_file: Path, parameters: list[str], elevated: bool) -> None:
    """Start the launcher script that kills StructureFinder and installs the new version.
...
def _write_launcher_script(setup_file: Path, parameters: list[str]) -> Path | None:
    """Write the script that ends StructureFinder and starts the installer next to the installer."""
...
def _launcher_script_text(setup_file: Path, parameters: list[str]) -> str:
    ...
    ``taskkill`` reports a non-zero exit code once no StructureFinder is left, which ends the
    wait loop.  Neither pipes nor ``start`` nor ``timeout`` are used, because the launcher runs
...
def _start_launcher(script: Path) -> None:
    """Run the launcher hidden and detached, so it survives the end of StructureFinder."""
...
def _start_launcher_elevated(script: Path) -> None:
    ...
    The UAC prompt is answered before StructureFinder quits, so a refusal can still be reported.
    """
    if is_elevated():
        # A child of an elevated StructureFinder is elevated as well, no UAC prompt needed:
        _start_launcher(script)
        return
...
def _command_processor() -> str:
    """cmd.exe of the system directory, which no StructureFinder installation can ever lock."""
...
def create_running_mutex() -> None:
    """Announce the running StructureFinder to Inno Setup (AppMutex)."""
